...
class _ChatScreenState extends State<ChatScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Always fetch fresh data when screen loads
  ...
  void didChangeDependencies() {
    ...
    // This will ensure data is refreshed when returning to this screen
    WidgetsBinding.instance.addPostFrameCallback((_) {
            ...
            Row(
              children: [
                InkWell(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => CreateGroupFlow())).then((_) {
                      // Refresh data when returning from create group flow
                      Provider.of<ChatProvider>(context, listen: false)
                          .fetchChatRooms();
                    });
                  },
                ...
                InkWell(
                  ...
                  child: Container(
                    padding:
                        EdgeInsets.symmetric(horizontal: 12.w, vertical: 5),
                    decoration: BoxDecoration(
                        color: Color(0xFF7400A5),
                        borderRadius: BorderRadius.circular(50.sp)),
                    child: Text(
              ...
              TabBar(
                ...
                tabs: [
                  ...
                  FittedBox(
                    child: Tab(
                      child:
                          Text('My Groups (${chatProvider.getGroupCount()})'),
                    ),
                  ),
  ...
  Widget _buildChatList(
      {required bool isDM, required ChatProvider chatProvider}) {
    return Consumer<ChatProvider>(
      builder: (context, provider, child) {
        final rooms =
            _isSearching ? provider.filteredChatRooms : provider.chatRooms;
        final filteredRooms = rooms
            .where((room) =>
                isDM ? room.roomType == 'dm' : room.roomType == 'group')
            .toList();
              ...
              leading: CircleAvatar(
                backgroundImage: room.roomType == 'dm'
                    ? room.participants.first.profilePic != null
                        ...
                        : const AssetImage('assets/avatar/2.png')
                            as ImageProvider
                    : room.groupProfile == null
                        ? const AssetImage('assets/avatar/2.png')
                        : NetworkImage(room.groupProfile!) as ImageProvider,
              ),
              title: Text(
                isDM
                    ? room.participants.first.name ?? 'Anonymous'
                    : room.groupName ?? 'Unnamed Group',
                style: GoogleFonts.roboto(
                  color: Theme.of(context).brightness == Brightness.dark
                      ? Colors.white
                      : Colors.black,
