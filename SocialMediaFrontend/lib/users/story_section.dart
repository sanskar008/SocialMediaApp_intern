...
class StorySection extends StatefulWidget {
  final String userId;
  final String token;
  StorySection({Key? key, required this.token, required this.userId}) : super(key: key);
...
class StorySectionState extends State<StorySection> {
  ...
  Future<Map<String, String>?> joinLiveStream(String toid , ) async {
  ...
  Future<void> fetchStories() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          stories = _parseStories(data['stories']) as List<Map<String, List<Story_Item>>>;
          print('xxxxxxx');
              ...
              children: [
                ListTile(
                    ...
                    onTap: () async {
                      ...
                      final result = await joinLiveStream(authorId , );
  ...
  void _showMYStory(BuildContext context, String authorId, List<Story_Item> stories) {
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    if (isLoading) {
      return Container(height: 120.h, child: _buildShimmerLoading());
      return Container(height: 120.h, child: _buildShimmerLoading());
    }
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemCount: stories.isEmpty ? 1 : stories.length + 1, // +1 for "Your Story" avatar
        itemBuilder: (context, index) {
          ...
          //           widget.userId,
          //           widget.token,
          //           _pickImage, // Callback to pick an image
          //           _showMYStory, // Callback to view existing stories
          //         );
          //       },
          //       child: FittedBox(
          //         child: Column(
          ...
          //             Text(
          //               'Your Story',
          //               style: GoogleFonts.roboto(color: Theme.of(context).brightness == Brightness.dark ? AppColors.darkText : AppColors.lightText, fontSize: 12.sp, fontWeight: FontWeight.w500),
          //             ),
          //           ],
          //         ),
          //       ),
          ...
          if(index == 0 ){
            // Inside the itemBuilder when index == 0
            return Padding(
              padding: EdgeInsets.only(right: 16),
              child: GestureDetector(
                onTap: () {
                    ...
                    children: [
                      Stack(
                        children: [
                          CircleAvatar(
                            radius: 30,
                            backgroundColor: Theme.of(context).brightness == Brightness.dark ? Colors.grey[800] : Colors.grey[500],
                            backgroundImage: NetworkImage(userProvider.userProfile!), // Replace with actual profile pic URL
                          ),
                          Positioned(
                            bottom: 0,
                            right: 0,
          ...
          return Padding(
            ...
            child: GestureDetector(
              onTap: () => 
              _showStory(context, authorId, storyItems, livehai!),
              child: StoryAvatar(
