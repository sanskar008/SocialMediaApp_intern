...
import 'package:socialmedia/utils/constants.dart'; // You'll need to add this dependency
...
class ReactionsProvider extends ChangeNotifier {
  ...
  Future<void> fetchReactions(
      String entityId, String entityType, String userId, String token) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var reaction in data['reactions']) {
          ...
          final userHasReacted =
              reactionData.users.any((user) => user.id == userId);
          if (userHasReacted) {
  ...
  Future<void> addReaction(String entityId, String entityType,
      String reactionType, String userId, String token) async {
    // Optimistically update UI
    _userReactionsByEntity[entityId] = reactionType;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Refresh reactions to get accurate counts
        await fetchReactions(entityId, entityType, userId, token);
      } else {
  ...
  void _optimisticallyUpdateReaction(
      String entityId, String type, String userId) {
    final reactions = _reactionsByEntity[entityId];
    ...
    for (var reaction in reactions) {
      if (reaction.type == type) {
        reaction.count += 1;
        reaction.users.add(UserData(id: userId, name: 'You', profilePic: null));
      } else if (reaction.users.any((u) => u.id == userId)) {
        // remove user from old reaction
        reaction.count -= 1;
        reaction.users.removeWhere((u) => u.id == userId);
      }
    }
    notifyListeners();
  }
  ...
  Future<void> removeReaction(
      String entityId, String entityType, String userId, String token) async {
    // Store previous reaction for rollback if needed
    final previousReaction = _userReactionsByEntity[entityId];

    // Optimistically update UI
    _userReactionsByEntity[entityId] = null;
    _optimisticallyRemoveReaction(entityId, userId);
    notifyListeners();
  ...
  void _optimisticallyRemoveReaction(String entityId, String userId) {
    final reactions = _reactionsByEntity[entityId];
    if (reactions == null) return;

    for (var reaction in reactions) {
      final userIndex = reaction.users.indexWhere((u) => u.id == userId);
      if (userIndex != -1) {
        reaction.users.removeAt(userIndex);
        reaction.count = (reaction.count - 1).clamp(0, double.infinity).toInt();
      }
    }
...
class _ReactionButtonState extends State<ReactionButton> {
  ...
  Future<void> _fetchReactions() async {
    ...
    await provider.fetchReactions(
        widget.entityId, widget.entityType, widget.userId, widget.token);
  }
  ...
  void _handleTap() {
    ...
    if (currentReaction != null) {
      ...
      provider.addReaction(widget.entityId, widget.entityType, 'like',
  ...
  void _showReactionPicker(BuildContext context) {
    ...
    final double leftPosition =
        position.dx - 50; // Center aligned with some offset
          ...
          child: Consumer<ReactionsProvider>(
            builder: (context, provider, _) {
              final currentReaction =
                  provider.getUserReactionForEntity(widget.entityId);
              return FloatingReactionPicker(
                onReactionSelected: (type) {
                  if (currentReaction == type) {
                    provider.removeReaction(widget.entityId, widget.entityType,
                        widget.userId, widget.token);
                  } else {
                    provider.addReaction(widget.entityId, widget.entityType,
                        type, widget.userId, widget.token);
                  }
