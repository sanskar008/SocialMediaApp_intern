...
class FollowerFollowingScreen extends StatefulWidget {
  ...
  const FollowerFollowingScreen({Key? key, this.initialTabIndex = 0})
      : super(key: key);
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    style: GoogleFonts.roboto(color: Theme.of(context).brightness ==
                                          Brightness.dark
                                      ? AppColors.darkText
                                      : AppColors.lightText),
                    decoration: InputDecoration(
                      hintText: "Search ",
                      hintStyle: GoogleFonts.roboto(color: Theme.of(context).brightness ==
                                          Brightness.dark
                                      ? AppColors.darkText
                                      : AppColors.lightText),
  ...
  Widget _buildListView({
    ...
    if (data.isEmpty) {
      return Center(
        child: Text(
          "No users found",
          style: GoogleFonts.roboto(fontSize: 14, color: Theme.of(context).brightness ==
                                          Brightness.dark
                                      ? AppColors.darkText
                                      : AppColors.lightText),
        ),
        ...
        return InkWell(
          onTap: () {
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        UserProfileScreen(userId: user['_id'])));
          },
          child: ListTile(
            ...
            trailing: Container(
              ...
              child: TextButton(
                onPressed: () async {
                  ...
                  try {
                    final url = isFollowerTab
                        ? Uri.parse("${BASE_URL}api/follower/remove")
                        : Uri.parse("${BASE_URL}api/unfollow");
                    ...
                    if (response.statusCode == 200) {
                      ...
                      print(
                          "Failed to ${isFollowerTab ? 'remove follower' : 'unfollow'}: ${responseString.body}");
                      Fluttertoast.showToast(
                        msg:
                            "Failed to ${isFollowerTab ? 'remove follower' : 'unfollow'}",
                        toastLength: Toast.LENGTH_SHORT,
                      );
                    }
                  } catch (e) {
                    print(
                        "Error ${isFollowerTab ? 'removing follower' : 'unfollowing'}: $e");
                    Fluttertoast.showToast(
                      msg:
                          "Error ${isFollowerTab ? 'removing follower' : 'unfollowing'}",
                      toastLength: Toast.LENGTH_SHORT,
                    );
                  }
                },
